...
class PseudocodeVariablesData(val pseudocode: Pseudocode, private val bindingContext: BindingContext) {
    ...
    class VariablesForDeclaration(
        ...
        val allVars =
                if (other.isEmpty())
                    ...
                    LinkedHashSet(valsWithInitializer).also { it.addAll(other) }
    }
            ...
            if (instruction is VariableDeclarationInstruction) {
                ...
                if (!containsDoWhile &&
                        (variableDeclarationElement is KtParameter || variableDeclarationElement is KtObjectDeclaration ||
                            variableDeclarationElement.safeAs<KtVariableDeclaration>()?.isSimpleValWithInitializer(descriptor) == true)) {
    ...
    private fun KtVariableDeclaration.isSimpleValWithInitializer(descriptor: VariableDescriptor) =
            (!isVar && (initializer != null || safeAs<KtProperty>()?.delegate != null || this is KtDestructuringDeclarationEntry)) ||
            descriptor.safeAs<PropertyDescriptor>()?.let { bindingContext.get(BindingContext.BACKING_FIELD_REQUIRED, it) != true } == true
